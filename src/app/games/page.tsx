...
import { TriviaGame, DevQuest, CodeBotBattler, DevType, CssPuzzle } from '@/components/games';
...
export default function GamesPage() {
  ...
  const cssPuzzles = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'data/css-puzzles.json'), 'utf8'));
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <section className="mt-12 border-t border-gray-200 dark:border-gray-800 pt-12">
        <div className="mb-6 flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">CSS Puzzle Game</h2>
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Test your layout skills! Write actual CSS to position elements correctly on the screen.</p>
          </div>
        </div>

        <div className="w-full">
          <CssPuzzle puzzles={cssPuzzles} />
        </div>
      </section>
